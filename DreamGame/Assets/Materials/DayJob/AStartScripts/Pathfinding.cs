...
public class Pathfinding : MonoBehaviour
{
	...
	******************/
	...
	void FindPath(Vector3 startPos, Vector3 endPos)
	{
		...
		List<Node> openSet = new List<Node>();			//represents the  nodes that the algorithm has NOT EVER picked as the lowest Fcost. 
		...
		*/
		...
		{
			Node currentNode = openSet[0];				//set currentNode = to the one and only thing in openSet

			for (int i = 1; i< openSet.Count; i++)		//loop through all the nodes in openSet and see if they are better than currentNode.  First iteration this will do nothing
			{
			...
			openSet.Remove(currentNode);				//only the newly chosen currentNode gets removed.  All the others stay, because
														//later they may have the lowest Fcost. 
